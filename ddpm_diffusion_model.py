...
"""初始化并训练扩散模型 EnhancedDiffusionModel完整的扩散模型封装，包括训练和生成功能。”"""
class EnhancedDiffusionModel(nn.Module):
    ...
    def train_on_positive_samples(self, all_data, epochs=100, batch_size=64):
        """训练扩散模型 训练过程是只使用训练集中的正样本（y == 1 的节点）。
        模型通过一个“加噪-去噪”的过程，反复学习如何从一个纯随机噪声中，逐步还原出真实的正样本特征。训练结束后，这个模型就成了一个“正样本生成器”"""
        positive_vectors = []
        protein_contexts = []
    ...
    def generate_positive_sample(self, protein_context, num_samples=100, add_diversity=True):
        """生成正样本"""
