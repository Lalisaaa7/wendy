...
def calculate_class_ratio(dataset):
    """计算数据集的类别比例,功能：一个辅助函数，用于统计数据集中正类和负类节点的数量和比例。
    流程：它会遍历数据集中的每一个图（data），累加所有图中标签为 1（正类）和 0（负类）的节点数量，最后计算出正类节点在总节点数中的占比。"""
    total_pos = 0
    total_neg = 0
...
def split_dataset(dataset, test_ratio=0.2):
    ...
    if not dataset:
        return [], []
#首先，它会随机打乱整个数据集（random.shuffle）以避免数据偏差。然后，根据给定的 test_ratio.将一部分数据划为测试集，剩下的作为训练集。
...
def augment_dataset(dataset, diffusion_model, config):
    ...
    # 为每个蛋白质图生成新样本,是整个数据生成流程的核心。它利用训练好的扩散模型，为每个图创造出新的、合成的正类节点，以达到预设的类别平衡目标。
    for data in tqdm(dataset, desc="Augmenting proteins"):
        try:
            protein_context = data.protein_context.to(config.device) #遍历训练集中的每一个蛋白质图。

            # 计算当前比例 计算该图中当前的正类节点数 n_pos 以及根据配置 config.target_ratio 应该达到的目标数量 target_pos。
            ...
            # 调用 diffusion_model.generate_positive_sample(...)。扩散模型在这里执行“反向去噪”过程，从随机噪声中生成新样本。
            ...
            # 创建新节点 将新生成的节点特征 (new_x) 和它们的标签（全为 1）与原始图的节点特征和标签合并。
            ...
            # 创建新边重建图结构 (改进的KNN) 即每个节点都与其特征空间中最相似的 k 个节点相连。这确保了新生成的节点能被合理地整合进图结构中。
            updated_edge_index = create_knn_edges(updated_x, k=config.knn_k, max_samples=2000)

            # 创建增强后的图，将这个增强后的新图添加到一个列表中并返回。
...
def oversample_positive_nodes(dataset, config):
    """对正样本节点进行过采样，限制最大节点数 （作为扩散模型生成数据后的一个补充或微调步骤。
    它的主要作用是通过复制已有的正样本来增加其数量，并确保每个图的总节点数不超过一个最大限制。）"""
    oversampled_data = []
...
def main():
    try:
        ...
        # 设置随机种子，设置随机种子确保实验可重复性
        set_seed(config.seed)

        # 创建输出目录 创建输出目录保存结果和模型
        os.makedirs(config.output_dir, exist_ok=True)
        print(f"Created output directory: {config.output_dir}")

        # 加载数据集，使用自定义的ProteinDataset类加载蛋白质数据，每个蛋白质被表示为图结构，包含节点特征和标签
